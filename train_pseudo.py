...
def evaluate_model(model, opt, device):
    ...
    model.eval()
    with torch.no_grad():
        # === 修复: 适配最新的 AutoEnc_MoE 架构 ===
        # 获取实际模型 (处理 DataParallel)
        ...
        if hasattr(mod, 'shared_enc') and hasattr(mod, 'moe_layer'):
            # 情况1: 最新版 AutoEnc_MoE (对称共享 + MoE修正)
            
            # 卫星 (Gallery): 只走共享主干
            g_enc = mod.shared_enc(gall_feat.to(device))
            
            # 无人机 (Query): 共享主干 + MoE修正 (Delta)
            ...
            # 情况2: 旧版 AutoEnc / AutoEnc2_share / 旧MoE
            # 注意: 有些旧代码 enc1 返回 tuple，有些返回 tensor，需兼容
...
def train(model, dataloader, criterions, optimizer, scheduler, scaler, recorder, device, opt):
    ...
    # 获取初始阈值 (如果没有设置则默认为 0.1)
    base_thr = getattr(opt.train, 'pseudo_thr', 0.1)

    for epoch in range(opt.train.epochs):
        # === 动态阈值逻辑 ===
        # 随着训练进行线性增加 0.1
        progress = epoch / opt.train.epochs
        current_thr = base_thr + 0.1 * progress
        
        # 更新 Loss 函数中的阈值
        if hasattr(criterions, 'thr'):
            criterions.thr = current_thr
        ...
        pbar = tqdm(enumerate(dataloader), total=len(dataloader), 
                    desc=f'Epoch {epoch+1}/{opt.train.epochs} [Thr:{current_thr:.3f}]')
        ...
        for batch_idx, data in pbar:
            ...
            if 'L_moe' in recorder.meter_dict:
                postfix_dict['Lmoe'] = f"{recorder.meter_dict['L_moe']['meter'].avg:.4f}"
...
def main():
    ...
    if torch.cuda.is_available():
        if ',' in gpu_str or gpu_str == '':
            # 多卡/空值: 使用屏蔽模式，device 设为相对索引 0
            os.environ['CUDA_VISIBLE_DEVICES'] = gpu_str
            device = 'cuda:0'
        else:
            # 单卡索引: 直接指定 device，不屏蔽
            try:
                gpu_idx = int(gpu_str)
    ...
    # === DataParallel 适配新的 GPU 逻辑 ===
    if torch.cuda.device_count() > 1 and ',' in gpu_str:
        model = torch.nn.DataParallel(model)
    
    model.to(device)

    recorder = Recorder({"FPS": 'd', "Loss": 'f', "L_moe": 'f'})
