...
def evaluate_model(model, opt, device):
    ...
    model.eval()
    with torch.no_grad():
        g_enc = model.enc1(gall_feat.to(device))
        q_enc = model.enc2(query_feat.to(device))
...
def train(model, dataloader, criterions, optimizer, scheduler, scaler, recorder, device, opt):
    for epoch in range(opt.train.epochs):
        ...
        pbar = tqdm(enumerate(dataloader), total=len(dataloader), desc=f'Epoch {epoch+1}/{opt.train.epochs}')
        for batch_idx, data in pbar:
            ...
            pbar.set_postfix({
                ...
                'recA': f"{recorder.meter_dict['Lrec_A']['meter'].avg:.4f}",
                'recB': f"{recorder.meter_dict['Lrec_B']['meter'].avg:.4f}",
