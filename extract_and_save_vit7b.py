...
def main():
    ...
    parser.add_argument('--device', type=str, default='cpu', choices=['cpu', 'cuda'], help='运行设备，默认 cpu（可选 cuda）')
    ...
    parser.add_argument('--save_dir', type=str, default='data/jzl/0-pipei-dinov3/feats', help='相对 HOME 的保存目录名')
    ...
    # 强制 CPU 运行（默认），仅当 --device=cuda 时才使用指定 GPU
    if str(args.device).lower() == 'cpu':
        os.environ['CUDA_VISIBLE_DEVICES'] = ''  # 禁用所有 CUDA 设备，避免占用显存
        run_device = 'cpu'
    else:
    ...
    print(f"[设备] 运行设备={run_device}，CUDA 可用={torch.cuda.is_available()}（已根据 --device 选项决定是否启用）")
